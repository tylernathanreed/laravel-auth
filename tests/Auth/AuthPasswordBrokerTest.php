...
use Mockery as m;
use Reed\Auth\Contracts\PasswordBroker;
...
class AuthPasswordBrokerTest extends PHPUnit_Framework_TestCase
{
    ...
    {
        $mocks = $this->getMocks();
        $broker = $this->getMock('Reed\Auth\Passwords\PasswordBroker', ['getUser', 'makeErrorRedirect'], array_values($mocks));
    ...
    {
        return new Reed\Auth\Passwords\PasswordBroker($mocks['tokens'], $mocks['users'], $mocks['mailer'], $mocks['view']);
    }

    protected function getMocks()
    {
        $mocks = [
            'tokens' => m::mock('Reed\Auth\Passwords\TokenRepositoryInterface'),
            'users'  => m::mock('Reed\Auth\Contracts\UserProvider'),
